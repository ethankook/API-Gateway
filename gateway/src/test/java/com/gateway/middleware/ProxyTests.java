...
class ProxyTests {
  ...
  void setUp() throws IOException {
    ...
    HttpClient httpClient =
        HttpClient.create().responseTimeout(Duration.ofMillis(properties.getReadTimeoutMs()));
  ...
  @Test
  void proxiesMatchedGetAndReturnsDownstreamStatusHeadersAndBody(CapturedOutput output)
      throws Exception {
    downstream.enqueue(
        new MockResponse()
            .setResponseCode(200)
  ...
  void returnsClean502JsonWhenDownstreamIsStopped() throws Exception {
    ...
    MockHttpServletRequest request =
        matchedRequest("GET", "/api/v1/leagues/42", "http://localhost:" + closedPort);
    MockHttpServletResponse response = new MockHttpServletResponse();
  ...
  void returnsClean504JsonWhenDownstreamExceedsTimeout() throws Exception {
    downstream.enqueue(
        new MockResponse().setHeadersDelay(1, TimeUnit.SECONDS).setBody("{\"late\":true}"));
  ...
  private MockHttpServletRequest matchedRequest(
      String method, String uri, String downstreamBaseUrl) {
