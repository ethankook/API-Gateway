...
public class RateLimiter {
    ...
    public RateLimitResult isAllowed(String key, Route route) {
        long capacity = route.getRateLimitCapacity() != null ? route.getRateLimitCapacity() : properties.getDefaultCapacity();
        long refillRate = route.getRateLimitRefillRate() != null ? route.getRateLimitRefillRate() : properties.getDefaultRefillRate();
        TokenBucket tokenBucket = tokenBuckets.computeIfAbsent(
                key,
                ignored -> new TokenBucket(capacity, refillRate, capacity, Instant.now())
        );
    ...
    private void refill(TokenBucket tokenBucket) {
        ...
        long nextTokenCount = Math.min(tokenBucket.getCapacity(), tokenBucket.getTokens() + replenishedTokens);
    ...
    public void cleanupTokenBuckets() {
        tokenBuckets.entrySet().removeIf(entry -> Duration.between(entry.getValue().getLastRefillTime(), Instant.now()).toMinutes() > 60);
    }
}
